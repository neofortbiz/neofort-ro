...
export async function generateMetadata({ params }) {
  ...
  return {
    title: isRo ? 'Contact NEOFORT | Bucuresti' : isUk ? 'Контакт NEOFORT | Бухарест' : 'Contact NEOFORT | Bucharest',
    description: isRo
...
export default async function ContactPage({ params }) {
  ...
  const carduri = [
    ...
    {
      title: isRo ? 'CREDITARE BANCARA SI NEBANCARA' : isUk ? 'БАНКІВСЬКЕ ТА НЕБАНКІВСЬКЕ КРЕДИТУВАННЯ' : 'BANK & NON-BANK FINANCING',
      img: '/images/Finantare_bancara_si_nebancara.avif',
      imgAlt: 'NEOFORT Creditare Bancara',
      href: 'mailto:office@neofort-biz.ro',
    },
    {
      title: isRo ? 'TAMPLARIE ALUMINIU SI TERMOPANE PVC' : isUk ? 'АЛЮМІНІЄВІ КОНСТРУКЦІЇ ТА ВІКНА ПВХ' : 'ALUMINIUM & PVC CARPENTRY',
  ...
  return (
    <div style={{ background: '#e8f1f2', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 20px 60px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        {carduri.map((card, i) => (
          <div key={i} style={{ background: '#fff' }}>
            ...
            <div style={{ padding: '24px 30px', textAlign: 'center' }}>
              <h2 style={{ fontFamily: F, fontSize: 16, fontWeight: 500, color: '#1a1a1a', letterSpacing: '0.08em', marginBottom: 20, textAlign: 'center', width: '100%' }}>
                {card.title}
              </h2>
              <a
                href={card.href}
                ...
                style={{ display: 'inline-block', padding: '10px 32px', border: '1px solid #1a1a1a', fontFamily: F, fontSize: 13, fontWeight: 300, color: '#1a1a1a', letterSpacing: '0.05em', borderRadius: 100, textDecoration: 'none' }}
              >
                {contactBtn}
              </a>
            </div>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '0 20px 0' }}>
        <h2 style={{ fontFamily: F, fontSize: 22, fontWeight: 200, color: '#1a1a1a', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 24, textAlign: 'center' }}>
