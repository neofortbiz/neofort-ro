    ...
    <header style={{ background: '#fff', borderBottom: '1px solid #efefef', position: 'sticky', top: 0, zIndex: 100 }}>
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Link href="/" locale={locale} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Image src="/images/Neofort.avif" alt="NEOFORT" width={64} height={64} style={{ objectFit: 'contain' }} />
          <div>
        ...
        <nav style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          {navLinks.map(({ href, label }) => (
            <Link key={href} href={href} locale={locale} style={{ fontFamily: 'Helvetica Neue, Helvetica, Arial, sans-serif', fontSize: 14, color: pathname === href ? '#1a1a1a' : '#4a4a4a', fontWeight: pathname === href ? 500 : 300, letterSpacing: '0.03em', textDecoration: 'none' }}>
              {label}
            </Link>
          ))}
          <div style={{ display: 'flex', gap: 6 }}>
            {otherLocales.map(l => (
              <Link key={l.code} href={pathname} locale={l.code} style={{ fontFamily: 'Helvetica Neue, Helvetica, Arial, sans-serif', fontSize: 12, color: '#4a4a4a', border: '1px solid #ccc', padding: '4px 10px', borderRadius: 3, letterSpacing: '0.05em', fontWeight: 400, textDecoration: 'none' }}>
                {l.label}
              </Link>
            ))}
          </div>
